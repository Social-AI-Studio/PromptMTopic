...
class ChatGPT_Base(Base_Model):
    def __init__(self) -> None:
        ...
        self.collect_examples=self.config['chatGPT']['collect_examples']
...
class ChatGPT(ChatGPT_Base):
    def __init__(self) -> None:
        ...
        self.group_examples=self.config['chatGPT']['group_examples']
    ...
    def group_topics(self,topic_docs,dataset,output_folder):
        output_dir=os.path.join(output_folder,dataset)
        os.makedirs(output_dir, exist_ok=True)
        output_path= os.path.join(output_folder,dataset,"topics_grouped.txt")
    ...
    def map_topics_to_k(self,topic_docs,dataset,top_k,output_folder):
        mapping={}
        all_topics={topic:len(files) for topic,files in topic_docs.items()}  
        all_topics=sorted(all_topics.items(), key = lambda x:x[1], reverse = False)
        all_topics=[item[0] for item in all_topics]
        group_file="{}/{}/groups_all.txt".format(output_folder,dataset)
        ...
        with open(os.path.join(output_folder,dataset,"mapping.json"),"w") as f:
            json.dump(mapping_ordered,f) 
        return mapping_ordered    
    
    def map_files_to_groups(topic_docs,mapping,dataset,output_folder):
        ...
        with open(os.path.join(output_folder,dataset,"grouped_ktopic_docs.json"),"w") as f:
            json.dump(topic_docs,f)

    def get_topic_rep(self,k_range,dataset,input_folder,output_folder):
        with open(os.path.join(output_folder,dataset,"grouped_ktopic_docs.json"),"r") as f:
    ...
    def order_topic_rep(self,dataset,output_folder):
        ...
        for topic,words in tqdm.tqdm(topic_rep.items()):
            if(topic in covered):
                index+=1
                continue
            ...
            with open(os.path.join(output_folder,dataset,"topic_rep_ordered.txt"),"a+") as f:
                json.dump({"topic":topic,"words":response},f)
                f.write("\n")
            index+=1     

    def clean_topic_rep(self,dataset,output_folder):
        with open(os.path.join(output_folder,dataset,"topic_rep.txt"),"r") as f:
        ...
        with open(os.path.join(output_folder,dataset,"topic_rep_ordered.txt"),"r") as f:
            topic_rep=f.read().split("\n")
        topic_rep=[item for item in topic_rep if item!='']    
        ...
        with open(os.path.join(output_folder,dataset,"topic_rep_ordered.txt"),"w") as f:
    ...
    def collapse(self,dataset,k,input_folder,output_folder):
        ...
        self.map_files_to_groups(mapping,dataset,output_folder)
